...
import { loadSuccess, loadFailure, createUnconfirmedUserSuccess, loadSuccessGetUnconfirmedUserById, updateUnconfirmedUserSuccess } from './actions';
...
export function* updateUnconfirmedUser({ payload: { data } }: any) {
  ...
  try {
    const response: AxiosResponse = yield call(apiSollar.put, `/unconfirmeduser/${_id}/update`, { ...data }, { headers: { token } });
...
export function* searchUnconfirmedUser({ payload: { value } }: any) {
  try {
    const response: AxiosResponse = yield call(apiSollar.get, `/unconfirmeduser/?limit=10&page=1${!!value ? '&search=' + value : ''}`)
    yield put(loadSuccess(response.data))
  } catch (error) {
