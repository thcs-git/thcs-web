...
export function* get({ payload }: any) {
  ...
  const response: AxiosResponse = yield call(apiSollar.get, `/specialty?limit=${params.limit ?? 1000}&page=${params.page || 1}${params.search ? '&search=' + params.search : ''}`);
...
export function* searchSpecialty({ payload: { value } }: any) {
  try {
    const response: AxiosResponse = yield call(apiSollar.get, `/specialty/?limit=10&page=1${!!value ? '&search=' + value : ''}`)
    yield put(loadSuccess(response.data))
  } catch (error) {
