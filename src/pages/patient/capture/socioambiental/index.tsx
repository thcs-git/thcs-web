...
export default function SocioAmbiental(
  ...
  const {
    documentGroupSocioAmbiental: documentGroupState,
  ...
  const [scoreSocial, setScoreSocial] = useState<IScore>({
    total: 0,
    complexity: "Sem Complexidade",
    status: "",
  });
  const [scoreAmbiental, setScoreAmbiental] = useState<IScore>({
    total: 0,
    complexity: "Sem Complexidade",
    status: "",
  });
  ...
  const calculateScore = useCallback(() => {
    ...
    let partialScoreSocial = 0,
      countQuestionRejectSocial = 0;
    let partialScoreAmbiental = 0,
      countQuestionRejectAmbiental = 0;
    ...
    const getStatus = (scoreSocial: number, scoreAmbiental: number) => {
      if (
        !(scoreSocial < 7 || countQuestionRejectSocial > 0) &&
        !(scoreAmbiental < 2 || countQuestionRejectAmbiental > 0)
      ) {
        return "Elegível";
      } else {
        return "Não Elegível";
  ...
  const isDone = useCallback(() => {
    let documentObj = _.find(care?.documents_id, {
      document_group_id: { name: "Tabela Socioambiental" },
    });
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (care?.patient_id?._id && care?._id) {
      ...
      if (document?._id) {
        ...
        dispatch(
          actionDocumentSocioAmbientalStoreRequest(createDocumentParams)
        );
      }
    }
  }, [documentGroup, care]);
          ...
          <>
            <FormContent>
                    ...
                    {field.description === "4. Espaço físico" && (
                      <>
                        <ScoreTotalContent>
                          ...
                          <ScoreTotal>
                            {scoreSocial.total
                              ? `${scoreSocial.total} - ${scoreSocial.status}`
                              : "0 - Não Elegível"}
                          </ScoreTotal>
                        </ScoreTotalContent>
              ...
              <ScoreTotalContent>
                <ScoreLabel>SCORE:</ScoreLabel>
                <ScoreTotal>
                  {scoreAmbiental.total
                    ? `${scoreAmbiental.total} - ${scoreAmbiental.status}`
                    : "0 - Não Elegível"}
                </ScoreTotal>
              </ScoreTotalContent>
                      ...
                      <FormGroup>
                        {field.options.map((option: any, index: number) => (
                          <FormControlLabel
                            ...
                            value={option._id}
                            onChange={(e) =>
                              selectOption(field._id, option._id, true)
                            }
                            control={
                              <Checkbox
                                color="primary"
