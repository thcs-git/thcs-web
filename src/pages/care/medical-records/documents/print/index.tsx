...
import {
  loadDocumentRequest as getDocumentById,
  loadCareById,
} from "../../../../../store/ducks/cares/actions";
import {
  DocumentGroupFields,
  DocumentGroupFiledsOptions,
...
export default function PrintDocument(props: IPageParams) {
  ...
  const handleOptionsAnswer = useCallback(
    (optionId: string): boolean => {
      const { document } = careState;

      const founded = document.fields.find(
        (field: any) => field.option_id === optionId
      );
  ...
  const renderHeader = useCallback(
    () => (
      <View wrap>
        ...
        >
          {careState?.document?.document_group_id?.name || ``}
        </Text>
        <View
          style={{
            fontSize: 11,
    ...
    <PDFComponent
      ...
      body={
        <View style={{ paddingRight: 20 }} wrap>
          {careState?.document?.document_group_id?.fields &&
            careState?.document?.document_group_id?.fields.map(
              (field: DocumentGroupFields, index: number) => (
                <Question key={`question_${index}`}>
                  <View wrap>
                    {(field.type === "radio" || field.type === "check") &&
                      field.options &&
                      field?.options.map(
                        (option: DocumentGroupFiledsOptions, index: number) => {
                          if (index === 0) {
                            return (
