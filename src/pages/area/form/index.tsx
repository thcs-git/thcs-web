...
import {
  AreaInterface,
  UserAreaInterface,
  NeighborhoodAreaInterface,
  CityAreaInterface,
  AreaTypes,
  ProfessionAreaInterface,
} from "../../../store/ducks/areas/types";
import {
  loadRequest,
  loadAreaById,
  updateAreaRequest,
  createAreaRequest,
  loadPointsArea,
  loadGetDistricts as getDistrictsAction,
  loadGetCitys as getStatesAction,
  loadGetDistricts_,
} from "../../../store/ducks/areas/actions";
import {
  loadRequest as getUsersAction,
...
export default function AreaForm(props: RouteComponentProps<IPageParams>) {
  ...
  const professionState = useSelector(
    (state: ApplicationState) => state.profession
  );
  ...
  useEffect(() => {
    if (params.id) {
      ...
      const dayOfTheWeekSelected =
        daysOfTheWeek.find((day) => day.id === areaState.data.week_day) || null;
  ...
  function goToNextTab(currentTab: Number) {
    switch (currentTab) {
      case 0:
        if (
          (inputName.error == false && state.supply_days == 1) ||
          (inputName.error == false && inputDays.error == false)
        ) {
        ...
        break;
      case 1:
        if (state.neighborhoods.length > 0) selectTab(2);
        break;
    }
  }
  //////////////////////////////// VALIDATION ///////////////////////////////////////////////////
  ...
  function handleSaveFormArea() {
    if (params.id) {
      if (
        ...
        (state.supply_days === 0 || inputDays.error == false) &&
  ...
  async function handleDeleteNeighborhood(
    neighborhood: NeighborhoodAreaInterface
  ) {
    if (canEdit) {
      let neighborhoodsSelected = [...state.neighborhoods];

      const neighborhoodFounded = neighborhoodsSelected.findIndex(
        (item: any) => {
  ...
  function handleSelectProfession(value: any) {
    value
      ? dispatch(getUsersAction({ profession_id: value._id }))
      : dispatch(getUsersAction());
  }
  function handleSelectUser(value: any) {
    const foundProfession = state.profession_users.findIndex(
    ...
    if (foundProfession > -1) {
      ...
      let prof: ProfessionAreaInterface = {
  ...
  function handleDeleteUser(
    ...
    if (canEdit) {
      ...
      if (professionFounded > -1) {
        const userFound = professionSelected[professionFounded].users.findIndex(
          (item: any) => {
          ...
          <FormContent>
            ...
            >
              ...
              {((params.id && params.mode == "view" && !canEdit) ||
                (params.id && params.mode == "create" && !canEdit)) && (
                <Button
            ...
            <TabContent>
              <TabNav>
                <TabNavItem
                  ...
                  onClick={() => goToNextMenu(0)}
                >
                  Dados da Área
                </TabNavItem>
                <TabNavItem
                  className={currentTab === 1 ? "active" : ""}
                  onClick={() => goToNextMenu(1)}
                >
                  <Badge
                    badgeContent={state.neighborhoods.length}
                    max={99}
                    color="primary"
                  >
                    {`Bairros`}
                ...
                <TabNavItem
                  ...
                  onClick={() => goToNextMenu(2)}
                >
                  <Badge
                    badgeContent={countProfessions()}
                    max={99}
                    color="primary"
                  >
                    {`Prestadores`}
                ...
                <TabNavItem
                  ...
                  onClick={() => goToNextMenu(3)}
                >
                  <Badge
                    badgeContent={areaState.points.length}
                    max={99}
                    color="primary"
                  >
                    {`Pacientes`}
                    ...
                    <Grid item md={4} xs={12}>
                      <FormGroupSection>
                        <TextField
                          ...
                          disabled={!canEdit}
                          helperText={
                            inputName.error && "Entre com um nome válido"
                          }
                        />
                      </FormGroupSection>
                    </Grid>
                            ...
                            <Autocomplete
                              ...
                              renderInput={(params) => (
                                <TextField
                                  {...params}
                                  autoFocus
                                  error={inputDays.error}
                                  label="Dia da semana"
                                  variant="outlined"
                                  onBlur={handleDaysValidator}
                                  helperText={
                                    inputDays.error && "Selecione um dia válido"
                                  }
                                />
                              )}
                  ...
                  <Grid item md={4} xs={12}>
                    {state.neighborhoods[0] && (
                      <DivideTitle>
                        ...
                        {state.neighborhoods[0]?.city ?? null}
                      </DivideTitle>
                    )}
                    {!state.neighborhoods[0] && (
                      <FormGroupSection>
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              ...
                              helperText={
                                inputState.error && "Selecione um estado válido"
                              }
                            />
                          )}
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              ...
                              label="Cidades"
                              variant="outlined"
                              onBlur={handleCityValidator}
                              InputProps={{
                                ...params.InputProps,
                                endAdornment: (
                                  <React.Fragment>
                                    {!areaState.citys[0] && load ? (
                                      <CircularProgress
                                        color="inherit"
                                        size={20}
                                      />
                              ...
                              helperText={
                                inputCity.error && "Selecione uma cidade válida"
                              }
                            />
                          )}
                          size="small"
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              ...
                              helperText={
                                inputCity.error && "Selecione um bairro válido"
                              }
                            />
                          )}
                          size="small"
                          ...
                          onChange={(event, value) => {
                            if (value) {
                              handleSelectUser({
                                _id: value._id || "",
                                name: value.name,
                                profession: value.profession_id?.name || "",
                              });
                            }
                          }}
                          fullWidth
