...
describe("Login validation", () => {
  ...
  it("Should to valid login Elements", () => {
    ...
    cy.get(loc.LOGIN.EMAIL).should("be.visible").and("be.empty");
    cy.get(loc.LOGIN.PASSWORD).should("be.visible").and("be.empty");
    cy.get(loc.LOGIN.REMEMBER).should("be.visible").and("not.be.checked");
    cy.xpath(loc.LOGIN.XP_BTN_ENTRAR).should("be.visible");
    cy.xpath(loc.LOGIN.XP_BTN_CRIAR_CONTA).should("be.visible");
    cy.xpath(
      `//p[contains(.,'Bem-vindo(a)! Realize seu login para continuar:')]`
    ).should("be.visible");
    cy.xpath(
      `//p[contains(.,'Bem-vindo(a)! Realize seu login para continuar:')]`
    ).should("be.visible");
    cy.xpath(`//*[@id="root"]/div[1]/div[1]/form/div[3]/div/p`).should(
      "be.visible"
    );
    cy.xpath(`//span[contains(.,'Lembrar de mim neste computador')]`).should(
      "be.visible"
    );
  });
